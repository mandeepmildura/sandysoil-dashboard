...
export default function ResetPassword() {
  ...
  // Supabase processes the recovery token in the URL hash and emits
  // PASSWORD_RECOVERY before the component can interact with the session.
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === 'PASSWORD_RECOVERY') setReady(true)
    })
    // Cover the case where the event already fired before this effect ran.
    ...
    <div className="min-h-screen bg-[#f9f9f9] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#ffffff] rounded-xl shadow-card p-7">
          <h1 className="font-headline font-bold text-xl text-[#1a1c1c] mb-1">Choose new password</h1>
          ...
          {!ready ? (
            <div className="flex items-center gap-3 py-4">
              ...
              <span className="text-sm font-body text-[#40493d]">Verifying reset link…</span>
            </div>
          ) : (

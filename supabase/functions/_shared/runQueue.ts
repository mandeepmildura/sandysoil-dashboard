 ...
 *
 * Drains every program_queue row whose fire_at falls inside the look-ahead
 * window, claims them so concurrent invocations can't double-fire, then waits
 * out each fire_at and publishes the MQTT commands.
 *
 * Used by:
 *   - run-program-queue (every-minute pg_cron tick — catches OFF steps and
 *     anything else that wasn't fired inline by the scheduler)
 *   - run-schedules     (called inline after queueing so newly-queued ON
 *     steps fire in the same minute as the scheduled time, instead of waiting
 *     for the next pg_cron tick)
 */
...
/**
 * Drain every program_queue row whose fire_at is at or before now+65s and
 * publish the corresponding MQTT commands. Idempotent: rows are claimed up
 * front via fired_at, so calling this twice in quick succession is safe.
 */
export async function runQueue(deps: RunQueueDeps): Promise<RunQueueResult> {
  ...
  // Claim every row up-front so the next pg_cron tick (or a concurrent
  // invocation from run-schedules) can't double-publish the same step.
  // Trade-off: a mid-run crash leaves these rows marked fired even though
  // no MQTT was sent — surfaced via the "Queue executor error" alert path.
  ...
  // A6v3 firmware doesn't report completion, so close the most recent open
  // zone_history row for each A6v3 off step we just published.
